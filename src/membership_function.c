 ...
 *
 * The triangular membership function is defined by three parameters a, b, and
 * c, which represent the start, peak, and end points of the triangle,
 * respectively.
 *
...
double triangularMembershipFunction(double x, double a, double b, double c) {
    ...
    // If x is on the left side of the triangle, calculate the membership degree
    // using the slope of the left side
    else if (x <= b) {
        if (b - a == 0) {
            return 1.0;
        } else {
    ...
    // If x is on the right side of the triangle, calculate the membership
    // degree using the slope of the right side
    else {
        return (c - x) / (c - b);
    }
}
 ...
 * and d, which represent the start, peak start, peak end, and end points of
 * the trapezoid, respectively.
 *
 * @param x The input value to calculate the membership degree for.
 ...
 * @return The membership degree of the input value.
 */
double trapezoidalMembershipFunction(double x, double a, double b, double c,
                                     double d) {
    // If x is outside the trapezoid, return 0 (no membership)
    if (x <= a || x >= d) {
        return 0.0;
    }
    // If x is on the left side of the trapezoid, calculate the membership
    // degree using the slope of the left side
    else if (x <= b) {
        return (x - a) / (b - a);
    }
    // If x is on the right side of the trapezoid, calculate the membership
    // degree using the slope of the right side
    else if (x >= c) {
        return (d - x) / (d - c);
    }
...
double membershipFunction(double x, MembershipFunction mf) {
    ...
    case TRAPEZOIDAL:
        // Call the trapezoidal membership function with the input x and the
        // membership function parameters
        return trapezoidalMembershipFunction(x, mf.a, mf.b, mf.c, mf.d);
    case RECTANGULAR:
        // Call the rectangular membership function with the input x and the
        // membership function parameters
        return rectangularMembershipFunction(x, mf.a, mf.b);
    default:
        // If the membership function type is not recognized, return 0 (no
        // membership)
        return 0.0;
    }
}
