...
#define CREATE_FUZZY_RULE_1(input1, input1_index, output_class, output_index)  \
    (FuzzyRule) {                                                              \
        ...
        .inputIndices = (int[]){input1_index}, .numInputs = 1,                 \
        .output = output_class, .outputIndex = output_index                    \
    }

#define CREATE_FUZZY_RULE_2(input1, input1_index, input2, input2_index,        \
                            output_class, output_index)                        \
    (FuzzyRule) {                                                              \
        .inputs = (FuzzyClass *[]){input1, input2},                            \
        .inputIndices = (int[]){input1_index, input2_index}, .numInputs = 2,   \
        .output = output_class, .outputIndex = output_index                    \
    }

#define CREATE_FUZZY_RULE_3(input1, input1_index, input2, input2_index,        \
                            input3, input3_index, output_class, output_index)  \
    (FuzzyRule) {                                                              \
        .inputs = (FuzzyClass *[]){input1, input2, input3},                    \
        .inputIndices = (int[]){input1_index, input2_index, input3_index},     \
        .numInputs = 3, .output = output_class, .outputIndex = output_index    \
    }
